...
public:
    ...
    //    loadSounds(); // Load the saved sound file paths on startup
        cuckooPlayer->setMedia(QUrl::fromLocalFile("/Applications/cuckooClock.app/Contents/MacOS/cuckoo.wav"));
//cuckooAudioOutput->setVolume(100);
        cuckooPlayer->durationChanged(1);
        playCuckoo(5);
        createTrayIcon();
    }
...
private slots:
    ...
    void playCuckoo(int count = 1) {
        if (cuckooPlayer->media().isNull()) return;
        cuckooPlayer->play();
qDebug()<<"Song duration: " << cuckooPlayer->duration();
    ...
    void playHalfHourChime() {
        if (halfHourPlayer->media().isNull()) return;
        halfHourPlayer->play();
    }
    ...
    void selectSound() {
        ...
        if (!soundFilePath.isEmpty()) {
            cuckooPlayer->setMedia(QUrl::fromLocalFile(soundFilePath));
            saveConfig();
        }
    }

    void selectHalfHourChime() {
        QString soundFilePath = QFileDialog::getOpenFileName(this, "Select Half-Hour Chime Sound", "/Applications/cuckooClock.app/Contents/MacOS/", "Sound Files (*.wav)");
        if (!soundFilePath.isEmpty()) {
            halfHourPlayer->setMedia(QUrl::fromLocalFile(soundFilePath));
            saveConfig();
        }
    }
    ...
    void saveConfig() {
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            out << cuckooPlayer->currentMedia().canonicalUrl().toString() << "\n";
            out << halfHourPlayer->currentMedia().canonicalUrl().toString() << "\n";
    ...
    void loadConfig(QSlider *slider) {
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            if (!cuckooSoundPath.isEmpty() && QFile::exists(cuckooSoundPath)) {
                cuckooPlayer->setMedia(QUrl::fromLocalFile(cuckooSoundPath));
                cuckooPlayer->setPosition(0); // Ensure the file is loaded
            }
            if (!halfHourSoundPath.isEmpty() && QFile::exists(halfHourSoundPath)) {
                halfHourPlayer->setMedia(QUrl::fromLocalFile(halfHourSoundPath));
